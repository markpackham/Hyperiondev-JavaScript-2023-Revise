...
example below, "Tom" is put at index 0 of the array jsStudents. "Sue" is at index 1, "Joe" is at index 2 etc.
...
// a space is created in memory to store the values assigned to that variable. An array creates enough space in
...
// ========= Example 3 =========
//Here's an example of how we would access a single element in the array. We do so by using the index that the element is at
...
// ========= Example 4 =========
//What is the name of the student at index 4 of jsStudents? Uncomment the code below and execute the code to
//see if you are correct

// console.log("Example 4: ", jsStudents[4]);
...
Trying to access an element with an index that is out of bounds is a very common error for which you should watch out */
...
it is easy to write a for loop that will loop through each element. With arrays, we can use a 
...
for (let i = 0; i < arrayLength; i++) {
  console.log(
    jsStudents[i] + " scored a grade of " + jsGrades[i] + "% in JavaScript"
  );
}
console.log();
...
console.log(
  "The result of adding all the elements in jsGrades using a for loop is " + sum
);
